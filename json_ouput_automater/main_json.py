...
def generate_testCase_for_testCaseList(testcase, content_list, parameter_list):
    ...
    for item in permuted_action_from_list:      
        ...
        for content in content_list:
            ...
            # print('count:', count)
            '''
            Changed the below hardcoded assignation to dynamic assgination with 'for' loop for final_lis
            # output_single_testcase_seqList.append(final_lis[0][0])
            # output_single_testcase_seqList.append(content_dict)
            # output_single_testcase_seqList.append(final_lis[1][0])
            '''
...
if __name__ == '__main__':
    ...
    for i in range(len(testcase_list)):
        ...
        outputJSONSheet["cases"] = testcases_list
